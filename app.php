...
$app->post('/api/products/{Route}/{Arrival}/{Departure}', function ($request, $response, $args) use ($productCollection) {
 ...
 $app->post('/api/products', function ($request,$response) use ($productCollection) {
     ...
     if (empty($data['Route']) || empty($data['Arrival']) || empty($data['Departure']) || empty($data['price'])) {
 ...
 $app->post("/api/products/modify", function($request, $response) use ($productCollection) {
     ...
     $name = isset($data['Route']) ? $data['Route'] : null;
     $arriv = isset($data['Arrival']) ? $data['Arrival'] : null;
     $depart = isset($data['Departure']) ? $data['Departure'] : null;
     $price = isset($data['price']) ? $data['price'] : null;
 
     // Validate the incoming data
     if (empty($name) || empty($arriv) || empty($depart) || empty($price)) {
     ...
     try {
         ...
         $product = $productCollection->findOne(['Route' => $name, 'Arrival' => $arriv]);
